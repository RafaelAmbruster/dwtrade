...
import com.dwtrade.application.config.KafkaProperties;
...
class DwTradeKafkaResourceIT {
    ...
    void setup() {
        ...
        DwTradeKafkaResource kafkaResource = new DwTradeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dw-trade-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dw-trade-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
